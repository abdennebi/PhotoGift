...
package photogift.server.controller;
...
public class TextSearchServlet extends HttpServlet {
  ...
  private static final Logger LOG = Logger.getLogger(TextSearchServlet.class.getName());
  ...
  private String add(HttpServletRequest req, User currentUser) {
    ...
    Document.Builder docBuilder = Document.newBuilder()
        ...
        .addField(Field.newBuilder().setName("email").setText(currentUser.getEmail()))
        .addField(Field.newBuilder().setName("domain").setText(currentUser.getAuthDomain()))
    ...
    if (tagStr != null) {
      ...
      while (tokenizer.hasMoreTokens()) {
        docBuilder.addField(Field.newBuilder().setName("tag").setAtom(tokenizer.nextToken()));
      }
    }
    ...
    try {
      ...
      for (ScoredDocument scoredDoc : results) {
        ...
        Document derived = Document.newBuilder()
            ...
            .addField(Field.newBuilder().setName("content").setText(content))
            .addField(Field.newBuilder().setName("nickname").setText(author.getNickname()))
            .addField(Field.newBuilder().setName("published").setDate(scoredDoc.getOnlyField("published").getDate()))
            .build();
        found.add(derived);
      }
    } catch (RuntimeException e) {
   ...
   *
   * @param req request received by the servlet
